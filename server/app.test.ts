...
describe('API', () => {
  ...
  it('validates and records one clarification after feedback', async () => {
    const store = new InMemoryScanStore()
    const runner = new ScanRunner(store, { retrieve: async () => source }, { analyze: async () => analysis, adapt: async () => ({ adaptedRecommendation: { title: 'Narrow the build', explanation: 'Prioritize Activity.', changedBecause: 'The builder has limited time.', sourceUrls: [source.url], confidence: 'high' }, nextClarifyingQuestion: 'Which evidence view matters most?' }) })
    const job = await runner.create({ hackathonUrl: source.url, builderContext: 'I want a trustworthy builder research report.', projectUrls: [] })
    await runner.run(job.id)
    await runner.applyFeedback(job.id, { feedback: 'Prioritize the shortest credible demo path.' })
    const app = createServerApp({ runner, store })
    const invalid = await request(app).post(`/api/scans/${job.id}/clarification`).send({ answer: '' })
    expect(invalid.status).toBe(400)
    expect(invalid.body.error).toBe('INVALID_CLARIFICATION')
    const response = await request(app).post(`/api/scans/${job.id}/clarification`).send({ answer: 'The Activity view matters most.' })
    expect(response.status).toBe(200)
    expect(response.body.feedback[0].clarificationResponse.answer).toContain('Activity')
    expect((await request(app).post(`/api/scans/${job.id}/clarification`).send({ answer: 'A second answer.' })).status).toBe(409)
  })
})
