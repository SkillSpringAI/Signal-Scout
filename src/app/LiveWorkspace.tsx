...
export function LiveWorkspace({ view, onNavigate, onSummaryChange }: { view: LiveView; onNavigate(view: LiveView): void; onSummaryChange(summary: LiveWorkspaceSummary): void }) {
  ...
  const recordClarification = async (answer: string) => {
    if (!job) return
    setError('')
    try { setJob(await api.submitClarification(job.id, answer)) }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Clarification could not be recorded.') }
  }
  ...
  if (view === 'memory') return <LiveContext job={job} error={error} applyFeedback={applyFeedback} recordClarification={recordClarification} />
...
export function LiveContext({ job, error, applyFeedback, recordClarification }: { job: ScanJob; error: string; applyFeedback(feedback: string): Promise<void>; recordClarification(answer: string): Promise<void> }) {
  ...
  const submitClarification = async () => { if (clarification.trim().length < 2) return; setSubmitting(true); try { await recordClarification(clarification) } finally { setSubmitting(false) } }
  const latest = job.feedback?.[job.feedback.length - 1]
  return <div className="stack-layout"><article className="panel"><div className="live-banner"><strong>GUIDED FEEDBACK</strong><span>Explicit feedback is persisted with this scan</span></div><h3>Builder context</h3><p>{job.request.builderContext}</p><label className="field-label">What should Signal Scout adapt?<textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} disabled={Boolean(latest)} /></label><button className="primary" disabled={submitting || Boolean(latest) || feedback.trim().length < 10} onClick={submit}>{submitting ? 'Adapting…' : latest ? 'Feedback turn complete' : 'Apply feedback to one recommendation →'}</button>{error && <p className="error-notice">{error}</p>}<p className="section-lead">This bounded interaction adapts one recommendation and asks one targeted follow-up. It is not a general chat system or an automatic durable-memory grant.</p></article>{latest && <article className="panel"><p className="eyebrow">ADAPTED RECOMMENDATION</p><h3>{latest.adaptedRecommendation.title}</h3><p>{latest.adaptedRecommendation.explanation}</p><p><strong>Changed because:</strong> {latest.adaptedRecommendation.changedBecause}</p><span className="tag">{latest.adaptedRecommendation.confidence} confidence</span><h4>One targeted clarification</h4><p>{latest.nextClarifyingQuestion}</p>{latest.clarificationResponse ? <div className="clarification-recorded"><strong>Your clarification was recorded:</strong><p>{latest.clarificationResponse.answer}</p><small>Saved with this scan without another Gemini call.</small></div> : <form onSubmit={(event) => { event.preventDefault(); void submitClarification() }}><label className="field-label">Your clarification<input value={clarification} onChange={(event) => setClarification(event.target.value)} placeholder="Type an answer and press Enter" /></label><button className="primary" type="submit" disabled={submitting || clarification.trim().length < 2}>{submitting ? 'Recording…' : 'Record clarification →'}</button><p className="section-lead">This records your answer with the scan. It does not trigger another Gemini call.</p></form>}<div className="evidence-links">{latest.adaptedRecommendation.sourceUrls.map((url) => <a key={url} href={url} target="_blank" rel="noreferrer" title={job.sources.find((source) => source.url === url)?.title}>{evidenceLinkLabel(job.sources, url)} ↗</a>)}</div></article>}</div>
}
...
const isWithheldAnalysis = (job: ScanJob) => job.status === 'partial' && ['MODEL_FAILED', 'EVENT_SOURCE_UNAVAILABLE'].includes(job.error?.code ?? '') && job.sources.length > 0
...
function PartialAnalysisNotice({ job, retryAnalysis }: { job: ScanJob; retryAnalysis?: () => void }) {
  ...
  const eventUnavailable = job.error?.code === 'EVENT_SOURCE_UNAVAILABLE'
  const explanation = eventUnavailable
    ? `Signal Scout preserved ${job.sources.length} project source${job.sources.length === 1 ? '' : 's'}, but did not expose event requirements or recommendations because the official hackathon source was unavailable.`
    : `Signal Scout collected and preserved ${job.sources.length} public source${job.sources.length === 1 ? '' : 's'}, but did not expose recommendations because analysis validation detected an unsupported or conflicting claim. Source collection succeeded.`
  const canRetry = !eventUnavailable && Boolean(retryAnalysis) && !retryUsed
  return <article className="panel partial-notice"><p className="eyebrow">SAFE PARTIAL RESULT</p><h3>Sources collected; recommendations withheld</h3><p>{explanation}</p><div className="preserved-sources">{job.sources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" title={source.title}>{evidenceLinkLabel(job.sources, source.url)} ↗</a>)}</div>{canRetry && <div className="button-row"><button className="primary" onClick={retryAnalysis}>Retry analysis with the same sources →</button></div>}{retryUsed && <p className="section-lead">The single analysis retry has been used. Preserved sources remain available for review.</p>}{job.error && <details className="technical-detail"><summary>Technical details</summary><p>{job.error.message}</p></details>}</article>
}
